...
def GraficosPolares():
    ...
    if single_graph_mode.get():
        ...
        ani_single_polar = FuncAnimation(fig_single_polar, atualizar_single_polar, interval=100, blit=False, cache_frame_data=False)
...
# Funções de callback para os sliders do 3D
def update_pitch_slider(value):
    global slider_pitch_val, label_pitch_val, canvas_3d
    slider_pitch_val = float(value)
    if label_pitch_val:
        label_pitch_val.configure(text=f"Pitch: {slider_pitch_val:.1f}°")
    # Sempre força o redesenho quando o slider é movido, independentemente do checkbox
    if 'canvas_3d' in globals() and canvas_3d:
        canvas_3d.draw_idle()

def update_roll_slider(value):
    global slider_roll_val, label_roll_val, canvas_3d
    slider_roll_val = float(value)
    if label_roll_val:
        label_roll_val.configure(text=f"Roll: {slider_roll_val:.1f}°")
    if 'canvas_3d' in globals() and canvas_3d:
        canvas_3d.draw_idle()

def update_yaw_slider(value):
    global slider_yaw_val, label_yaw_val, canvas_3d
    slider_yaw_val = float(value)
    if label_yaw_val:
        label_yaw_val.configure(text=f"Yaw: {slider_yaw_val:.1f}°")
    if 'canvas_3d' in globals() and canvas_3d:
        canvas_3d.draw_idle()

# Função de callback para os checkboxes de sobreposição
def on_override_checkbox_change():
    """Força o redesenho do gráfico 3D quando um checkbox de sobreposição é alterado."""
    if 'canvas_3d' in globals() and canvas_3d:
        canvas_3d.draw_idle()
...
def Grafico3D():
    ...
    global slider_pitch_val, slider_roll_val, slider_yaw_val
    global label_pitch_val, label_roll_val, label_yaw_val
    global slider_pitch, slider_roll, slider_yaw # Referências aos widgets de slider
    # Override checkboxes
    global override_pitch_var, override_roll_var, override_yaw_var
    ...
    main_3d_frame = ctk.CTkFrame(master=janela)
    main_3d_frame.pack(fill="both", expand=True, pady=5, padx=10)
    widgets_criados.append(main_3d_frame)

    # Frame para o gráfico 3D
    graph_frame = ctk.CTkFrame(master=main_3d_frame)
    graph_frame.pack(side=tk.TOP, fill="both", expand=True)
    ...
    toolbar_3d = NavigationToolbar2Tk(canvas_3d, graph_frame)
    toolbar_3d.update()

    # Frame para os Sliders e Checkboxes (abaixo do gráfico)
    slider_control_frame = ctk.CTkFrame(master=main_3d_frame)
    slider_control_frame.pack(side=tk.BOTTOM, fill="x", padx=10, pady=10)
    widgets_criados.append(slider_control_frame)

    # Adicionar sliders para Pitch, Roll e Yaw
    # Pitch Slider e Checkbox
    pitch_row_frame = ctk.CTkFrame(slider_control_frame, fg_color="transparent")
    pitch_row_frame.pack(fill="x", pady=2)
    
    ctk.CTkCheckBox(pitch_row_frame, text="Manual", variable=override_pitch_var, command=on_override_checkbox_change).pack(side="left", padx=(0,5))
    ctk.CTkLabel(pitch_row_frame, text="Pitch:").pack(side="left", padx=(0, 5))
    slider_pitch = ctk.CTkSlider(pitch_row_frame, from_=-90, to=90, number_of_steps=180, command=update_pitch_slider)
    slider_pitch.set(slider_pitch_val) # Seta o valor inicial do slider
    slider_pitch.pack(side="left", fill="x", expand=True, padx=(0, 10))
    label_pitch_val = ctk.CTkLabel(pitch_row_frame, text=f"Pitch: {slider_pitch_val:.1f}°")
    label_pitch_val.pack(side="left")

    # Roll Slider e Checkbox
    roll_row_frame = ctk.CTkFrame(slider_control_frame, fg_color="transparent")
    roll_row_frame.pack(fill="x", pady=2)
    
    ctk.CTkCheckBox(roll_row_frame, text="Manual", variable=override_roll_var, command=on_override_checkbox_change).pack(side="left", padx=(0,5))
    ctk.CTkLabel(roll_row_frame, text="Roll:").pack(side="left", padx=(0, 5))
    slider_roll = ctk.CTkSlider(roll_row_frame, from_=-90, to=90, number_of_steps=180, command=update_roll_slider)
    slider_roll.set(slider_roll_val) # Seta o valor inicial do slider
    slider_roll.pack(side="left", fill="x", expand=True, padx=(0, 10))
    label_roll_val = ctk.CTkLabel(roll_row_frame, text=f"Roll: {slider_roll_val:.1f}°")
    label_roll_val.pack(side="left")

    # Yaw Slider e Checkbox
    yaw_row_frame = ctk.CTkFrame(slider_control_frame, fg_color="transparent")
    yaw_row_frame.pack(fill="x", pady=2)
    
    ctk.CTkCheckBox(yaw_row_frame, text="Manual", variable=override_yaw_var, command=on_override_checkbox_change).pack(side="left", padx=(0,5))
    ctk.CTkLabel(yaw_row_frame, text="Yaw:").pack(side="left", padx=(0, 5))
    slider_yaw = ctk.CTkSlider(yaw_row_frame, from_=0, to=360, number_of_steps=360, command=update_yaw_slider)
    slider_yaw.set(slider_yaw_val) # Seta o valor inicial do slider
    slider_yaw.pack(side="left", fill="x", expand=True, padx=(0, 10))
    label_yaw_val = ctk.CTkLabel(yaw_row_frame, text=f"Yaw: {slider_yaw_val:.1f}°")
    label_yaw_val.pack(side="left")


    # Add statistics frame for 3D graph
    stats_frame_3d = create_statistics_frame(main_3d_frame, "Estatísticas 3D (Pitch, Roll, Yaw)", single_3d_stats_labels)
    ...
    def update_3d(frame):
        # SEMPRE adquire dados para manter as listas globais (y_data, etc.) atualizadas,
        # mesmo que não sejam usados para plotar no 3D nesta iteração.
        atualizar_dados_para_graficos() 
        ...
        # Lógica para determinar qual valor usar (slider ou dados em tempo real)
        if override_pitch_var.get():
            pitch = slider_pitch_val
        else:
            # Garante que y_data não está vazio antes de tentar acessar o último elemento
            pitch = y_data[-1] if y_data else 0.0 # Valor padrão 0.0 se não houver dados
        
        if override_roll_var.get():
            roll = slider_roll_val
        else:
            roll = y_data2[-1] if y_data2 else 0.0
            
        if override_yaw_var.get():
            yaw = slider_yaw_val
        else:
            yaw = y_data3[-1] if y_data3 else 0.0
        
        # Os labels dos sliders já são atualizados nas funções de callback dos sliders
        # Eles refletem o valor do slider, não necessariamente o que está sendo plotado.
        # Se você quiser que o label mostre o valor plotado, ajuste o texto do label aqui.
        # Exemplo: label_pitch_val.configure(text=f"Pitch (Plot): {pitch:.1f}°")
        # No entanto, a requisição foi para alterar "entrada inicial", o label ao lado do slider
        # indica o valor do slider. Para um label do valor plotado, seria necessário outro widget.

        # ALTERAÇÃO AQUI: De 100 para 9 pontos para formar um octógono (já feito)
        ...
        # Update statistics for 3D view (considering the actual values used for plotting)
        current_angles = []
        if check_var1.get(): current_angles.append(pitch)
        if check_var2.get(): current_angles.append(roll)
        if check_var3.get(): current_angles.append(yaw)
        update_statistics_display(current_angles, single_3d_stats_labels)


        canvas_3d.draw_idle()

    # Inicia a animação. A animação agora apenas força o redesenho baseado nos valores do slider.
...
def mostrar_view(nome_view):
    ...
    if nome_view == 'tela_inicial':
        ...
        Graficos()
        # Se os controles do gráfico devem ser visíveis para esta view
        if controles_graficos_frame and controles_graficos_frame.winfo_exists():
...
def setup_main_window():
    global janela, controles_graficos_frame
    global random_data, angular_test_data, single_graph_mode # Movidas para cá
    global check_var1, check_var2, check_var3 # Movidas para cá
    global override_pitch_var, override_roll_var, override_yaw_var # Movidas para cá
    ...
    # Inicialização das variáveis de controle (BooleanVar) - AGORA AQUI
    ...
    # NOVAS VARIÁVEIS GLOBAIS PARA OS CHECKBOXES DE SOBREPOSIÇÃO - AGORA AQUI
    override_pitch_var = ctk.BooleanVar(master=janela, value=False)
    override_roll_var = ctk.BooleanVar(master=janela, value=False)
    override_yaw_var = ctk.BooleanVar(master=janela, value=False)
