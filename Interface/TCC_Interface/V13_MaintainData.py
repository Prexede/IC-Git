...
def Grafico3D():
    ...
    # x_data.clear(); y_data.clear(); ... # REMOVED: Data clearing
    parar_animacoes()
    limpar_area_principal()
    ...
    def update_3d(frame):
        ...
        if check_var1.get(): # Pitch (intermediário, vermelho)
            pitch_points = np.vstack([1.0 * np.cos(theta), np.zeros_like(theta), 1.0 * np.sin(theta)])
        ...
        if check_var2.get(): # Raw/Roll (interno, verde)
            roll_points = np.vstack([np.zeros_like(theta), 0.5 * np.cos(theta), 0.5 * np.sin(theta)])
