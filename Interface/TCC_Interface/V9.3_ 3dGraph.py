...
def Graficos():
    ...
    # Hide/show individual checkboxes based on single_graph_mode state
    # This is a bit tricky to do directly from here as widgets are packed
    # A better approach would be to manage this in `criar_controles_laterais`
    # or by unpacking/repacking them. For now, we will just redraw the graphs
    # based on the `single_graph_mode` state.
    ...
    if single_graph_mode.get(): # NEW: Single graph mode
        ...
        if lines_to_plot_in_legend:
            ...
            # If no lines are plotted, ensure no legend is shown
            if ax_single_linear.legend_:
                ax_single_linear.legend_.remove()
...
def GraficosPolares():
    ...
    if single_graph_mode.get(): # NEW: Single polar graph mode
        ...
        # Place legend outside for clarity, only for selected bars
...
def Graficos3D():
    ...
    theta_circle = np.linspace(0, 2 * np.pi, R_circle)
    ...
    # Inicializa os plots para os indicadores de ângulo atuais e opostos
    # Pitch indicators
    pitch_indicator_line, = ax_3d.plot([0,0], [0,0], [0,0], 'r-', linewidth=2)
    pitch_indicator_point, = ax_3d.plot([0], [0], [0], 'ro', markersize=8)
    pitch_opposite_line, = ax_3d.plot([0,0], [0,0], [0,0], 'r:', linewidth=1, alpha=0.7) # Dotted line for opposite
    ...
    def atualizar_3d(i):
        ...
        if len(y_data) > 0 and len(y_data2) > 0 and len(y_data3) > 0:
            ...
            # Calcula os ângulos opostos (adiciona 180 graus, e normaliza para 0-360 se necessário)
            P_opposite_rad = math.radians((current_pitch_deg + 180) % 360)
            Ra_opposite_rad = math.radians((current_raw_deg + 180) % 360)
            Y_opposite_rad = math.radians((current_yaw_deg + 180) % 360)
            ...
            opposite_Y_x = R_circle * math.cos(Y_opposite_rad)
            opposite_Y_y = R_circle * math.sin(Y_opposite_rad)
            yaw_opposite_line.set_data_3d([0, opposite_Y_x], [0, opposite_Y_y], [0, 0])
            yaw_opposite_point.set_data_3d([opposite_Y_x], [opposite_Y_y], [0])
...
def setup_main_window():
    global janela, controles_graficos_frame, random_data, angular_test_data, single_graph_mode, check_var1, check_var2, check_var3 # Adicionado angular_test_data e single_graph_mode
    ...
    janela.geometry(f"{largura_janela}x{altura_janela}+{pos_x}+{pos_y}") # FIX: Changed x and y to pos_x and pos_y
    ...
    single_graph_mode = ctk.BooleanVar(master=janela, value=False) # NEW: Default to multiple graphs
    check_var1 = ctk.BooleanVar(master=janela, value=True) # Default to showing Pitch
    check_var2 = ctk.BooleanVar(master=janela, value=True) # Default to showing Raw
    check_var3 = ctk.BooleanVar(master=janela, value=True) # Default to showing Yaw
    ...
    adicionar_botao(frame_botoes_nav, r'C:\Users\carlo\OneDrive\Documentos\Drive\Faculdade\IC\Interface\Background\chart.png', "Gráficos 3D", lambda: mostrar_view('graficos_3d')) # New 3D button
